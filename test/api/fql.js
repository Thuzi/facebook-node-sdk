...
describe('FB.api', function() {
	describe('FQL', function() {
		...
		describe("FB.api('fql', { q: [ 'SELECT uid FROM user WHERE uid=me()', 'SELECT name FROM user WHERE uid=me()' ] }, cb)", function() {
		...
		describe("FB.api('fql', { q: { id: 'SELECT uid FROM user WHERE uid=me()', name: 'SELECT name FROM user WHERE uid IN (SELECT uid FROM #id)' } }, cb)", function() {
			it('should execute a multi-query', function(done) {
				var expectedRequest = nock('https://graph.facebook.com:443')
